...
class Notification(TimeStampedModel,StatusModel,SoftDeletableModel):
    sender = models.ForeignKey(settings.AUTH_USER_MODEL,related_name='notifications_sender',on_delete=models.SET_NULL,null=True,blank=True,verbose_name='发送者')
    receiver = models.ForeignKey(settings.AUTH_USER_MODEL,related_name='notifications_receiver',on_delete=models.SET_NULL,null=True,blank=True,verbose_name='接收者')
    STATUS = Choices('unread','read')
    message = models.TextField(blank=True,verbose_name='消息内容')
    description = models.CharField(max_length=50,null=True,blank=True,verbose_name='描述')
